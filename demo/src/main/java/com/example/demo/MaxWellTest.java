...
public class MaxWellTest implements Runnable {
  protected MaxwellConfig config;
  protected MaxwellContext context;
  protected Replicator replicator;

  static final Logger LOGGER = LoggerFactory.getLogger(MaxWellTest.class);

  public MaxWellTest(MaxwellConfig config) throws SQLException, URISyntaxException {
    this.config = config;
  ...
  private Position attemptMasterRecovery() throws Exception {
    ...
    if (recoveryInfo != null) {
      Recovery masterRecovery = new Recovery(config.replicationMysql, config.databaseName,
          this.context.getReplicationConnectionPool(), this.context.getCaseSensitivity(), recoveryInfo);

      recoveredPosition = masterRecovery.recover().getPosition();

      if (recoveredPosition != null) {
        // load up the schema from the recovery position and chain it
        // into the
        // new server_id
        MysqlSchemaStore oldServerSchemaStore = new MysqlSchemaStore(context.getMaxwellConnectionPool(),
            context.getReplicationConnectionPool(), context.getSchemaConnectionPool(),
            recoveryInfo.serverID, recoveryInfo.position, context.getCaseSensitivity(), config.filter,
            false);
  ...
  private void startInner() throws Exception {
    ...
    mysqlSchemaStore.captureAndSaveSchema().getSchema(); // trigger schema to load / capture before
    // we start the replicator.

    // this.replicator = new BinlogConnectorReplicator(mysqlSchemaStore,
    // producer, bootstrapper, this.context, initPosition);

    this.replicator = new BinlogConnectorReplicator(mysqlSchemaStore, producer, bootstrapper,
        this.config.maxwellMysql, this.config.replicaServerID,
        this.config.databaseName, this.context.getMetrics(),
        initPosition, false, this.config.clientID, this.context.getHeartbeatNotifier(), this.config.scripting);
  ...
  public static MaxwellConfig generateMaxwellConfig(Map<String,String> properties) {
    List<String> configList = new ArrayList<>();
    for (Map.Entry<String,String> entry : properties.entrySet()) {
      configList.add("--" + entry.getKey());
      configList.add(entry.getValue());
    }
    String[] argv = new String[configList.size()];
    return new MaxwellConfig(configList.toArray(argv));
